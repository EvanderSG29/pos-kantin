...
export function renderNavbar({ title, session }) {
  const { user, useMockApi } = session;
  const roleLabel = user.role === "admin" ? "Administrator" : "Petugas";
  const apiBadgeClass = useMockApi ? "warning" : "success";
  const apiBadgeText = useMockApi ? "Mock API" : "Apps Script Live";
    ...
    <nav class="navbar navbar-expand navbar-light bg-white topbar shadow pos-topbar">
      ...
      <ul class="navbar-nav ml-auto align-items-center">
        <li class="nav-item d-none d-sm-block mr-3">
          <span class="badge badge-pill badge-${apiBadgeClass} px-3 py-2 shadow-sm">${escapeHtml(apiBadgeText)}</span>
        </li>
